...
<form action="/SiteCourses_war/record-courses" method="post">
    <% List<Cours> courses = (List<Cours>) request.getAttribute("courses");
    for(Cours cours: courses){
%>
    <legend>Выберите курсы</legend>
    <fieldset>
        <div>
            <label for="<%=cours.getStyle()%>"><%=cours.getStyle()%></label>
            <input type="radio" name="dance" id="<%=cours.getStyle()%>" value="<%=cours.getStyle()%>">
        </div>
